...
namespace NetworkSniffer.ViewModel
{
    ...
    public class MainViewModel : ViewModelBase
    {
        ...
        private bool monitorWithNpcap = true;
        public bool MonitorWithNpcap
        {
            get
            {
                return monitorWithNpcap;
            }
            set
            {
                monitorWithNpcap = value;
                GetInterfaces();
            }
        }
        #endregion
        ...
        private void GetInterfacesUsingNpcap()
        {
            var devices = CaptureDeviceList.Instance;

            foreach (LibPcapLiveDevice device in devices)
            {
                if (device.Interface.Addresses.Count > 0 && device.MacAddress != null)
                {
                    InterfaceList.Add(new IPNetworkInterface
                    {
                        InterfaceAddress = device.Interface.MacAddress.ToString(),
        ...
        private List<string> ValidIPAddress(List<string> IPList, string isValid)
        {
            ...
            else if (Regex.Match(isValid, SrcPatternV6).Success ||
                     Regex.Match(isValid, DstPatternV6).Success)
            {
        ...
        private void StartCaptureExecute()
        {
            ...
            else
            {
                try
                {
                    if (!monitorWithNpcap && monitor == null)
                    {
                    ...
                    if (monitorWithNpcap && monitorNpcap == null)
                    {
